...
************************************************************************************************************************
  ...
  * 14 May 2018 *
  * l_fetchVinData.cfm *
  * pushes data into the NHTSA vin decoder and returns csv file with info *
************************************************************************************************************************
--->
...
<!--- discovered algorithm from Ben Nadel --->
<!---<cffunction
    name="CSVToQuery"
    access="public"
    returntype="query"
...
<!---<cfset vinList = ValueList(qryVehicles.vin [, delimiter])/>
<cfloop query="#qryVehicles#">
  <cfset vinList = listappend(vinList, qryVehicles.vin, ";") />
</cfloop>

<cfhttp url="https://vpic.nhtsa.dot.gov/api/vehicles/DecodeVINValuesBatch/"
  name="VINDetails"
  method="get"
  timeout="10"
  resolveurl="false"
  ...
  <cfhttpparam name="data" type="formfield" value="#vinList#">
</cfhttp>

<!---<cfset qResult = CSVToQuery(
  CSV = CSVFileContent.Trim()
  ) />--->
...
<cfdump var="#cfhttp#" />--->
<cfscript>
  cfhttp(url="https://vpic.nhtsa.dot.gov/api/vehicles/DecodeVIN/" & qryVehicles.vin[1] & "?format=json", method="get", timeout="10", resolveurl="false", redirect="false" result="result");
  r = deserializeJSON(result.filecontent);
  WriteDump(var = r);
  p = r.results.find(function(i) { return compareNoCase(i.variable, "model year") == 0; });
  m = (p == 0) ? 'null' : '#r.results[p].value#';
  writeOutput('p: #p#, m:#m#<br />');
</cfscript>
